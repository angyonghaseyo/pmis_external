...
const CargoManifest = () => {
  ...
  const fetchManifests = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${API_URL}/cargo-manifests`, {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (currentManifest) {
        await axios.put(`${API_URL}/cargo-manifests/${currentManifest.id}`, submissionData, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        });
        setSnackbar({ open: true, message: 'Cargo manifest updated successfully', severity: 'success' });
      } else {
        await axios.post(`${API_URL}/cargo-manifests`, submissionData, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${API_URL}/cargo-manifests/${id}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
          ...
          <TableBody>
            {manifests.map((manifest) => (
              <TableRow key={manifest.id}>
                ...
                <TableCell>{new Date(manifest.departureDate).toLocaleString()}</TableCell>
                <TableCell>{new Date(manifest.arrivalDate).toLocaleString()}</TableCell>
